 ...
 *
 * Note that because there is a single `n0x7C7BD2C32785E7669C59AcE6c2656FecE7Fdbc6ConReentrant` guard, functions marked as
...
contract HornyHornyHippos is ERC721Enumerable, Ownable {
    ...
    address GeeksWallet;
    address BrennansWallet;
    address BeaversWallet;

    uint256 geeksCut = 10;
    uint256 brennansCut = 10;
    uint256 beaversCut = 10;
    ...
    function updateCuts(uint256 _Geeks, uint256 _Brennan, uint256 Beavers) public onlyOwner {
        geeksCut = _Geeks;
        brennansCut = _Brennan;
        beaversCut = Beavers;
    }

    function updateWallets(address _Geeks, address _Brennan, address _Beavers) public onlyOwner {
        GeeksWallet = _Geeks;
        BrennansWallet = _Brennan;
        BeaversWallet = _Beavers;
    }
    ...
    function adjustPills (uint256 _blueCost, uint256 _yellowCost, uint256 _blueEffect, uint256 _yellowEffect, string memory _newMessageBlue, string memory _newMessageYellow) public onlyOwner {
        littleBluePill = _blueCost;
        littleYellowPill = _yellowCost;
        bluePillEffect = _blueEffect;
        yellowPillEffect = _yellowEffect;
        blueMessage = _newMessageBlue;
        yellowMessage = _newMessageYellow;
    }
    ...
    function hippoSex (uint256 _IdMale, uint256 _IdFemale) public {
        ...
        require(nft[_IdFemale].ownerOf == msg.sender && nft[_IdMale].ownerOf == msg.sender, "You Don't Own Both Of Those Hippos");
        ...
        require(!nft[_IdMale].baby && !nft[_IdFemale].baby, "Horny Horny Hippos Rule #3: No Pedophilia / Underage Sex Allowed");
    ...
    function giveBirth (uint256 _IdFemale) public {
        require(nft[_IdFemale].babyDaddy != 0, "She Ain't Knocked Up");
        require(block.timestamp - nft[_IdFemale].lastHadSex >= gestationPeriod, "She Ain't Ready To Pop Yet");

        uint256 babyDaddy = nft[_IdFemale].babyDaddy;

        string calldata Gender = determineGender();

        uint256 ID = totalSupply() + 1;
        _safeMint(msg.sender, ID);
            nft[ID].iD = ID;
            nft[ID].ownerOf = msg.sender;
            nft[ID].gender = Gender;
            nft[ID].birthdate = block.timestamp;
            nft[ID].lastHadSex = 0;
            nft[ID].babyDaddy = 0;
            nft[ID].baby = true;
            nft[ID].cantBreedWith = cantHaveSexWith(Gender, babyDaddy, _IdFemale);
            nft[ID].color = determineColor(nft[babyDaddy].color, nft[_IdFemale].color);
            nft[ID].trait1 = determineTrait(nft[babyDaddy].trait1, nft[_IdFemale].trait1);
            nft[ID].trait2 = determineTrait(nft[babyDaddy].trait2, nft[_IdFemale].trait2);
            nft[ID].trait3 = determineTrait(nft[babyDaddy].trait3, nft[_IdFemale].trait3);
            nft[ID].trait4 = determineTrait(nft[babyDaddy].trait4, nft[_IdFemale].trait4);
            nft[ID].trait5 = determineTrait(nft[babyDaddy].trait5, nft[_IdFemale].trait5);

            if (viewPendingETHRewards(msg.sender) > 0 || viewPendingGeeksRewards(msg.sender) > 0) {
                ClaimRewards();
            }

            breeder[msg.sender].hipposBirthed ++;
            nft[_IdFemale].babyDaddy = 0;
            nft[_IdFemale].lastHadSex = 0;
    }
        
        function cantHaveSexWith(string calldata _gender, uint256 _father, uint256 _mother) internal returns (uint256) {
            if (_gender == "Male") {
                return _mother;
            }
        ...
        function determineGender() internal returns (string memory) {
            if (genderCounter == 10) {
                genderCounter = 1;
                return "Male";
            }
        ...
        function determineColor(string calldata _maleColor, string calldata _femaleColor) internal returns (string calldata) {
            if (_maleColor == "Red" & _femaleColor == "Red") {
                return "Red";

            if (keccak256(abi.encodePacked(nft[_Id].gender)) == keccak256(abi.encodePacked("Male"))) {
            }

            if (_maleColor == "Red" & _femaleColor == "Blue") {
                return "Red";
            }

            if (_maleColor == "Red" & _femaleColor == "Purple") {
                return "Red";
            }

            if (_maleColor == "Red" & _femaleColor == "Orange") {
                return "Red";
            }
            //

            if (_maleColor == "Blue" ** _femaleColor == "Red") {
                return "Red";
            }

            if (_maleColor == "Blue" & _femaleColor == "Blue") {
                return "Red";
            }

            if (_maleColor == "Blue" & _femaleColor == "Purple") {
                return "Red";
            }

            if (_maleColor == "Blue" & _femaleColor == "Orange") {
                return "Red";
            }
            //
            if (_maleColor == "Purple" & _femaleColor == "Red") {
                return "Red";
            }

            if (_maleColor == "Purple" & _femaleColor == "Blue") {
                return "Red";
            }

            if (_maleColor == "Purple" & _femaleColor == "Purple") {
                return "Red";
            }

            if (_maleColor == "Purple" & _femaleColor == "Orange") {
                return "Red";
            }
            //
            if (_maleColor == "Orange" & _femaleColor == "Red") {
                return "Red";
            }

            if (_maleColor == "Orange" & _femaleColor == "Blue") {
                return "Red";
            }

            if (_maleColor == "Orange" & _femaleColor == "Purple") {
                return "Red";
            }

            if (_maleColor == "Orange" & _femaleColor == "Orange") {
                return "Red";
            }
        }

        function determineTrait(uint256 _maleTrait, uint256 _femaleTrait) internal returns (uint256) {

            if (_maleTrait == 1 && _femaleTrait == 1) {
                return 1;
            }

            if (_maleTrait == 1 && _femaleTrait == 2) {
                return 1;
            }

            if (_maleTrait == 1 && _femaleTrait == 3) {
                return 1;
            }

            if (_maleTrait == 1 && _femaleTrait == 4) {
                return 1;
            }

            if (_maleTrait == 1 && _femaleTrait == 5) {
                return 1;
            }

            if (_maleTrait == 2 && _femaleTrait == 1) {
                return 1;
            }

            if (_maleTrait == 2 && _femaleTrait == 2) {
                return 1;
            }

            if (_maleTrait == 2 && _femaleTrait == 3) {
                return 1;
            }

            if (_maleTrait == 2 && _femaleTrait == 4) {
                return 1;
            }

            if (_maleTrait == 2 && _femaleTrait == 5) {
                return 1;
            }

            //
            if (_maleTrait == 3 && _femaleTrait == 1) {
                return 1;
            }

            if (_maleTrait == 3 && _femaleTrait == 2) {
                return 1;
            }

            if (_maleTrait == 3 && _femaleTrait == 3) {
                return 1;
            }

            if (_maleTrait == 3 && _femaleTrait == 4) {
                return 1;
            }

            if (_maleTrait == 3 && _femaleTrait == 5) {
                return 1;
            }
            //
            if (_maleTrait == 4 && _femaleTrait == 1) {
                return 1;
            }

            if (_maleTrait == 4 && _femaleTrait == 2) {
                return 1;
            }

            if (_maleTrait == 4 && _femaleTrait == 3) {
                return 1;
            }

            if (_maleTrait == 4 && _femaleTrait == 4) {
                return 1;
            }

            if (_maleTrait == 4 && _femaleTrait == 5) {
                return 1;
            }
            //
            if (_maleTrait == 5 && _femaleTrait == 1) {
                return 1;
            }

            if (_maleTrait == 5 && _femaleTrait == 2) {
                return 1;
            }

            if (_maleTrait == 5 && _femaleTrait == 3) {
                return 1;
            }

            if (_maleTrait == 5 && _femaleTrait == 4) {
                return 1;
            }

            if (_maleTrait == 5 && _femaleTrait == 5) {
                return 1;
            }
        }

    function takeBluePill (uint256 _IdMale) public payable returns(string calldata) {
        require(nft[_IdMale].ownerOf == msg.sender, "You can only gives drugs to your Hippos");
        require(!nft[_IdMale].baby, "You can't give drugs to babies");
        require(msg.value == littleBluePill, "Incorrect Ether amount sent");

        nft[_IdMale].lastHadSex - bluePillEffect;

        uint256 toGeeks = msg.value / geeksCut;
        uint256 toBrennan  = msg.value / brennansCut;
        uint256 toBeavers = msg.value / beaversCut;

       uint256 toHippos = (msg.value - toGeeks - toBrennan - toBeavers);

        GeeksWallet.transfer(toGeeks);
        BrennansWallet.transfer(toBrennan);
        BeaversWallet.transfer(toBeavers);
        address(this).transfer(toHippos);
    ...
    function takeYellowPill (uint256 _IdMale) public {
        require(nft[_IdMale].ownerOf == msg.sender, "You can only gives drugs to your Hippos");
        require(!nft[_IdMale].baby, "You can't give drugs to babies");
        require(msg.value == littleYellowPill, "Incorrect Ether amount sent");

        nft[_IdMale].lastHadSex - yellowPillEffect;
    ...
    function GrowUp (uint256 _HippoID) public {
        require(nft[_HippoID].ownerOf == msg.sender, "Thats not your Baby");
        require(nft[_HippoID].baby == true, "They are already grown up");
        require(block.timestamp - nft[_HippoID].birthdate >= growUpTime, "They aren't ready to grow up yet");

        nft[_HippoID].baby = false;
    }
   ...
   function claimEthRewards() internal {
        ...
        uint256[] memory ownedNfts = walletOfOwner(msg.sender);
            ...
            for (uint256 j = breeder[msg.sender].ethClaimNumber + 1; j < ethcurrentDepositNumber; j++) {
               ...
               uint256 perHippoBred = deposit.value2; 
                ...
                if (j < ethcurrentDepositNumber){
                    ...
                    breeder[msg.sender].EthtotalClaimed += beforeClaim;
                    breeder[msg.sender].ethClaimNumber++;  
                }
    ...
    function claimGeeksRewards() internal {
        ...
        for (uint256 i = 0; i < ownedNfts.length; i++) {
            ...
            for (uint256 j = breeder[msg.sender].geeksClaimNumber + 1; j < ethcurrentDepositNumber; j++) {
