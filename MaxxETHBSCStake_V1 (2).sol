...
contract MAXXStaking is Ownable {
    ...
    // set current rwds rate after interaction
   function calculateRwdsPerTknPerSec() internal {
    require(PWRDPoolStaked > 0, "Cannot divide by zero");
    calculateTimeRemain();

    // Calculate the raw currentRwdsRate
    uint256 rawRwdsRate = (PWRPool * 1e18) / (PWRDPoolStaked * PWRDPoolTimeRemain);

    // Update the currentRwdsRate with 18 decimal places
    currentRwdsRate = rawRwdsRate;

    // If you want to convert currentRwdsRate to wei, you can do this:
    // uint256 currentRwdsRateWei = currentRwdsRate * 1 wei;

    // Now currentRwdsRate contains the value with 18 decimal places
}
  ...
  function subtractPool() internal {
    ...
    // Calculate the raw amtToSub value without adjusting for decimal places
    uint256 amtToSubRaw = (PWRDPoolStaked * timeDif * currentRwdsRate);

    // Convert the raw amtToSub value to wei (18 decimal places)
    uint256 amtToSubWei = amtToSubRaw / 1e18;

    // Set the amtSub value in wei
    amtSub = amtToSubWei;

    require(currentBal >= amtToSubWei, "Insufficient balance for subtraction");

    // Perform the subtraction on PWRPool
...
//claiming rwds
    ...
    function DepositPWRDInWEI(uint256 amount) external onlyOwner {
        ...
        PWRPool = PwrdToken.balanceOf(address(this));
        timeDif = block.timestamp - lastAction;
        lastAction = block.timestamp;
        calculateRwdsPerTknPerSec();
