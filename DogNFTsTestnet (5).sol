...
//This code is for ERC-721 Collection of 300 "Pups" within Geeks Ecosystem
//These NFTs earn 1% of the volume on all $Geeks, $ETH, and $BNB paired Tokens
//The $BNB will be bridged over to Base before being deposited
//The code below enables a dual reward system ($ETH & $Geeks) that can show pending rewards on a dApp
//Incase the NFTs do not mint out prior to launch, a Mint Mechanism is included for minting after deployment
//An airdropper is built into the code to mint & distribute pre-bought NFTs 

pragma solidity ^0.8.0;

contract GeeksDeliveryPups is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using SafeMath for uint256;

    constructor(address Geeks, address _recieverOfFunds, string memory _initURI) ERC721("Geeks Pups", "Geeks Pup"){
        recieverWallet = address(_recieverOfFunds); //Wallet receiving mint funds
        payoutToken = IERC20(Geeks); 
        setURI(_initURI);
    }
     
//NFT Struct for tracking owner, $Geeks claims and $ETH claims
    struct NFT {
        uint256 iD;
        address ownerOf;
...
//Struct for tracking $Geeks Deposits
...
//Struct for tracking $Geeks Deposits
    ...
    uint256 public totalNftSupply = 0; //current supply of NFTs minted
    uint256 public maxSupply = 300; //Total number of NFTs mintable
    ...
    function setReceiverWallet(address _newAddress) public onlyOwner {
