...
contract RufferalCoupon is ERC721Enumerable, Ownable {
    ...
    mapping(address => ruffer) public ruffers;
    ...
    uint256 public RuffTaxes = 300;//The token cut when funding a coupon with tokens

    uint256 public ruffCut = 150;//The volume portion on buys to Ruff

    uint256 public rufferCut = 75;//The volume portion on buys to Affiliates //make the functions to change these

    uint256 public aevumCut = 75;//The volume portion on buys to Aevum DeFi
    
    address public RuffTeam = 0x3058026FaA939fa34046d11EfccDF3a83B2e0457;

    address public Aevum = 0x5C35e73A435359277eBDC97209f2BFe89852b13E;
    ...
    event CouponFunded(uint256 couponId, address Contract);
    event CouponBuy(uint256 couponId, uint256 amount);
  
    constructor(address _uniswapRouterAddress) ERC721("Rufferal Smart Coupon System", "Smart Coupon") {
    ...
    function updateRuffTaxes(uint256 newTaxes) external onlyOwner {
        require(newTaxes <= 500, "Taxes cannot exceed 500");
        emit RuffTaxesUpdated(newTaxes, RuffTaxes);
        RuffTaxes = newTaxes;
    }

    function updateRuffFeeReceiver(address newWallet) external onlyOwner {
        emit FeeReceiverUpdated(newWallet, RuffTeam);
        RuffTeam = newWallet;
    }

    function updateAevumFeeReceiver(address newWallet) external  {
        require(msg.sender == Aevum,"not aevum Wallet");
        emit FeeReceiverUpdated(newWallet, Aevum);
        Aevum = newWallet;
    }

    function addDays(uint256 timestamp, uint256 daysToAdd) internal pure returns (uint256) {//internal call used for hours
        return timestamp + (daysToAdd * 1 hours);
    }



     function viewUserCouponIds(address userAddress) public view returns (uint256[] memory) {
        return users[userAddress].couponIds;
    }
    ...
    function mintCoupon() external {
        ...
        if (ruffers[msg.sender].ruffer == address(0)) {
            ruffers[msg.sender].ruffer = msg.sender;
        }

        ruffer storage myRuffer = ruffers[msg.sender];
        myRuffer.couponIds[couponId].push(couponId);
    ...
    function fundNFT( 
        ...
        uint256 _tknVestTime, 
        bool _ethRewards 
    ) external payable { 

        uint256 tax = RuffTaxes / 100;
        ...
        require(_tknVestTime <= 336, "max 2 weeks vesting"); 
        ...
        if(_ethRewards == true){ 
            require(msg.value >= _startingRewards, "incorrect Eth Amount"); 
            payable(address(this)).transfer(_startingRewards); 
            coupons[_couponId].startingRewards = _startingRewards; 
        } 
 
        if(_ethRewards == false){ 
            IERC20 couponToken = IERC20(_contractAddress); 
            uint256 fullRwd = _startingRewards; 
            uint256 ruffTake = (fullRwd * RuffTaxes) /10000; 
            uint256 afterTaxAmount = fullRwd - ruffTake; 
            couponToken.approve(address(this), _startingRewards); 
            couponToken.transferFrom(msg.sender, address(this), _startingRewards); 
            couponToken.transfer(RuffTeam, ruffTake);    
            coupons[_couponId].startingRewards = afterTaxAmount; 
        } 
            coupons[_couponId].name = _name; 
            coupons[_couponId].contractAddress = _contractAddress; 
            coupons[_couponId].bonusPercentage = _bonusPercentage; 
            coupons[_couponId].expirationTime = block.timestamp + (1000 * 365 days); 
            coupons[_couponId].tknVestTime = _tknVestTime; 
            coupons[_couponId].ethRewards = _ethRewards; 
            emit CouponFunded(_couponId,_contractAddress);
    ...
    function buyWithCouponv2(uint256 couponId, uint256 ethAmount, uint256 slippage) external payable {
        ...
        ruffer storage Ruffers = ruffers[coupon.owner];
        ...
        require(buyer.contrct[couponId] == address(0), "Pending Rewards, Please Claim");
        
        uint256 totalTax = (ruffCut + rufferCut + aevumCut);
        uint256 AftrTx = (ethAmount * totalTax) / 10000;
        uint256 Taxed = (ethAmount - AftrTx);

        uint256 toAevum = (ethAmount * aevumCut) / 10000;
        uint256 toRuff = (ethAmount * ruffCut) / 10000;
        uint256 toRuffer = (ethAmount * rufferCut) / 10000;

        if(coupon.ethRewards == true) {
            viewBonusPayable(ethAmount,coupon);
        }     

        if(coupon.ethRewards == false) {
        ...
        transferToRuffTeam(toRuff);
        transferToAevum(toAevum);
        transferToRuffer(toRuffer, couponId);

        uint256 boughtAmount = swapETHForTokens(Taxed, slippage, coupon);

        if (coupon.ethRewards == true) {
            uint256 bonusPayable = calculateBonusPayable(Taxed, coupon);
            handleBonusPayableWithVesting(buyer, couponId, bonusPayable);
        }

        if (coupon.ethRewards == false) {
            uint256 bonusTokens = calculateBonusTokens(boughtAmount, coupon);
            handleBonusTokenWithVesting(buyer, couponId, bonusTokens);
        }

        Ruffers.volumeGenerated += ethAmount;
        Ruffers.ethMade += toRuffer;
        coupon.volume += ethAmount;
        emit CouponBuy(couponId,ethAmount);
    }

    function requestCloseCoupon(uint256 couponId) external {
        require(couponExists(couponId), "Coupon ID does not exist");
        Coupon storage coupon = coupons[couponId];
        require(msg.sender == coupons[couponId].funder);
        coupon.expirationTime = block.timestamp + 1 days;     ///Set back to days
    }

    function closeCoupon(uint256 couponId) external {
        require(couponExists(couponId), "Coupon ID does not exist");
        require(msg.sender == coupons[couponId].funder);
        require(coupons[couponId].expirationTime < block.timestamp);
        Coupon storage coupon = coupons[couponId];
       ...
       uint256 toFunder = (coupon.startingRewards - coupon.RewardsSent);
        ...
        if (coupon.ethRewards == true) {
            payable(msg.sender).transfer(toFunder);
        }

        if (coupon.ethRewards == false) {
            IERC20(coupon.contractAddress).transfer(coupon.funder, toFunder);
        }

        
        
        coupon.funder = address(0);
        coupon.contractAddress = address(0);
    ...
    function withdrawVestedTokens(uint256 couponId) external {
        ...
        address couponContract = buyer.contrct[couponId];
        uint256 balance = IERC20(couponContract).balanceOf(msg.sender);
        uint256 boughtBalance = buyer.holdings[couponId];
        uint256 boughtThreshold = (boughtBalance * 70) / 100;

        
        if (balance >= boughtThreshold) {
            IERC20 couponCont = IERC20(buyer.contrct[couponId]);
            couponCont.transfer(msg.sender, vestedAmt);
            ...
            buyer.contrct[couponId] = address(0);
        }

        else if (balance < boughtThreshold) {
            IERC20 couponCont = IERC20(buyer.contrct[couponId]);
            address couponOwner = coupons[couponId].owner;

            uint256 RuffAmt = (vestedAmt * RuffTaxes) / 10000;
            uint256 refundAmt = (vestedAmt - RuffAmt);
            couponCont.transfer(RuffTeam, RuffAmt);
            couponCont.transfer(couponOwner, refundAmt);
            buyer.status[couponId] = "You Sold & Lost Vesting Amount";
            buyer.unlockTime[couponId] = 0;
            buyer.holdings[couponId] = 0;
            buyer.vestingAmtTkn[couponId] = 0;
            buyer.contrct[couponId] = address(0);
        }
    ...
    function withdrawVestedEth(uint256 couponId) external {
        ...
        if (balance >= (boughtBalance * 70) / 100) {
        ...
        else if (balance < (boughtBalance * 70) / 100) {
            address couponOwner = coupons[couponId].owner;
    ...
    function GetCouponInfo(address couponcontract) public view returns (uint256[] memory, uint256[] memory) {
        uint256 totalSupply = totalSupply() + 1;
        uint256[] memory matchingIds = new uint256[](totalSupply);
        uint256[] memory bonusPercentages = new uint256[](totalSupply);
        uint256 count = 0;

        for (uint256 i = 1; i < totalSupply; i++) {
            if (coupons[i].contractAddress == couponcontract) {
                matchingIds[count] = i;
                bonusPercentages[count] = coupons[i].bonusPercentage;
                count++;
            }
        }

        // Resize the result arrays to the actual number of matching IDs
        uint256[] memory resultIds = new uint256[](count);
        uint256[] memory resultBonusPercentages = new uint256[](count);
        for (uint256 j = 0; j < count; j++) {
            resultIds[j] = matchingIds[j];
            resultBonusPercentages[j] = bonusPercentages[j];
        }

        return (resultIds, resultBonusPercentages);
    }
    ...
    function emergencyTokenWithdrawFunds(uint256 couponId, address funder) external onlyOwner {
        require(couponExists(couponId), "Coupon ID does not exist");
        Coupon storage coupon = coupons[couponId];

        // Calculate the remaining funds in the coupon
        uint256 remainingFunds = (coupon.startingRewards - coupon.RewardsSent);

        // Transfer the remaining funds to the contract owner
        IERC20(coupon.contractAddress).transfer(coupon.owner, remainingFunds);
    ...
    function viewUserInfo(address account,uint256 tokenId) public view returns (
        address owner,
        address contractAddress,
        ...
        owner = user.owner;
        contractAddress = user.contrct[tokenId];
        vestingAmtTkn = user.vestingAmtTkn[tokenId];
        vestingAmtEth = user.vestingAmtEth[tokenId];
        unlockTime = user.unlockTime[tokenId];
        bought = user.bought[tokenId];
        holdings = user.holdings[tokenId];
        status = user.status[tokenId];
    }

    function calculateBonusPayable(uint256 amount, Coupon storage coupon) internal view returns (uint256) {
        uint256 bonusPayable = (amount * coupon.bonusPercentage) / 100;
        return bonusPayable;
    }
    ...
    function swapETHForTokens(uint256 amount, uint256 slippage, Coupon storage coupon) internal returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapRouter.WETH();
        path[1] = coupon.contractAddress;
        ...
        uint256 beforeBalance = IERC20(coupon.contractAddress).balanceOf(msg.sender);
        ...
        uint256 afterBalance = IERC20(coupon.contractAddress).balanceOf(address(msg.sender));
        uint256 boughtAmount = afterBalance - beforeBalance;

        return boughtAmount;
    }

    function handleBonusPayableWithVesting(User storage buyer, uint256 couponId, uint256 bonusPayable) internal {
        Coupon storage coupon = coupons[couponId];
        buyer.owner = msg.sender;
        buyer.couponIds.push(couponId);
        buyer.contrct[couponId] = coupon.contractAddress;
        buyer.vestingAmtEth[couponId] = (buyer.vestingAmtEth[couponId] + bonusPayable);
        buyer.unlockTime[couponId] = addDays(block.timestamp, coupon.tknVestTime);
        buyer.bought[couponId] = true;
        coupon.RewardsSent = (coupon.RewardsSent + bonusPayable);
    ...
    function handleBonusTokenWithVesting(User storage buyer, uint256 couponId, uint256 bonusTokens) internal {
        Coupon storage coupon = coupons[couponId];
        buyer.owner = msg.sender;
        buyer.couponIds.push(couponId);
        buyer.contrct[couponId] = coupon.contractAddress;
        buyer.vestingAmtTkn[couponId] = (buyer.vestingAmtTkn[couponId] + bonusTokens);
        buyer.unlockTime[couponId] = addDays(block.timestamp, coupon.tknVestTime);//Tripple Check
        buyer.bought[couponId] = true;
        coupon.RewardsSent = (coupon.RewardsSent + bonusTokens);
    ...
    function viewBonusPayable(uint256 amount, Coupon storage coupon) internal view returns (uint256) {
        uint256 bonusPayable = (amount * coupon.bonusPercentage) / 100;
        uint256 remaining = coupon.startingRewards;
        uint256 sent = (coupon.RewardsSent + bonusPayable);
    ...
    function transferToRuffTeam(uint256 amount) internal {
        payable(RuffTeam).transfer(amount);
    }

    function transferToAevum(uint256 amount) internal {
        payable(Aevum).transfer(amount);
    }

    function transferToRuffer(uint256 amount, uint256 couponId) internal {
        Coupon storage coupon = coupons[couponId];
        payable(coupon.owner).transfer(amount);
    }
